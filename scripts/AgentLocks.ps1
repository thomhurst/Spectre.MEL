...
# resolves from -OwnerId, SPECTRE_MEL_AGENT_LOCK_OWNER_ID, or CODEX_THREAD_ID (in that order).
...
#   $env:SPECTRE_MEL_AGENT_LOCK_OWNER_ID = 'my-agent-run-id'
...
$stateRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'spectre-mel-agent-locks'
...
function Resolve-OwnerIdentity([string]$Explicit) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:SPECTRE_MEL_AGENT_LOCK_OWNER_ID)) {
        return $env:SPECTRE_MEL_AGENT_LOCK_OWNER_ID.Trim()
    }
...
if (-not $OwnerId) {
    Die 2 'cannot resolve lock owner identity: pass -OwnerId or set SPECTRE_MEL_AGENT_LOCK_OWNER_ID (CODEX_THREAD_ID is used automatically in Codex)'
}
...
$lockKey = "spectre-mel:agent-lock:$LockName"
$metaKey = "spectre-mel:agent-lock-meta:$LockName"
