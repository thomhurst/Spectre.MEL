...
$redisContainer = 'spectre-mel-agent-locks-redis'
$suffix = [Guid]::NewGuid().ToString('N')
$issueNumber = Get-Random -Minimum 100000 -Maximum 999999
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) "spectre-mel-agent-lock-owner-tests-$suffix"
...
function Invoke-AgentLocks {
    ...
    $previousOwnerId = [Environment]::GetEnvironmentVariable('SPECTRE_MEL_AGENT_LOCK_OWNER_ID', 'Process')
    $previousThreadId = [Environment]::GetEnvironmentVariable('CODEX_THREAD_ID', 'Process')

    try {
        [Environment]::SetEnvironmentVariable('SPECTRE_MEL_AGENT_LOCK_OWNER_ID', $EnvironmentOwnerId, 'Process')
        ...
        [Environment]::SetEnvironmentVariable('SPECTRE_MEL_AGENT_LOCK_OWNER_ID', $previousOwnerId, 'Process')
...
function Expire-TestLock([string]$LockName) {
    $deleted = & docker exec $redisContainer redis-cli DEL "spectre-mel:agent-lock:$LockName" "spectre-mel:agent-lock-meta:$LockName"
...
function Assert-OwnerIdentityNotExposed([string]$LockName, [string]$OwnerId) {
    $meta = & docker exec $redisContainer redis-cli GET "spectre-mel:agent-lock-meta:$LockName"
...
function Remove-TestLocks([string[]]$LockNames) {
    $keys = @($LockNames | ForEach-Object { "spectre-mel:agent-lock:$_"; "spectre-mel:agent-lock-meta:$_" })
