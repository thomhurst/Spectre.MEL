...
- Sequence: implement → run focused + broader tests → `/simplify` → commit referencing `#<N>` → `git push --force-with-lease` → confirm the PR is opened/updated on GitHub. Only after the push lands is the unit complete.
...
$worktreeRoot = Join-Path (Split-Path $repo -Parent) "Spectre.MEL-worktrees"
...
If the sandbox cannot write to the sibling directory, use `C:\tmp\Spectre.MEL-worktrees` instead.
...
Spectre.MEL tracks decomposition with GitHub **native sub-issues** (parent/child) and **dependencies** (`blocked_by` / `blocking`) when available, not markdown checklists as source of truth. Prefer native `blocked_by` relationships for blockage. Repo is `thomhurst/Spectre.MEL`. Write calls need the numeric issue `.id` (an integer distinct from `#number`, passed with `-F`, not `-f`).
...
```powershell
gh api repos/thomhurst/Spectre.MEL/issues/<N>/sub_issues --paginate --jq '.[] | "\(.number) [\(.state)] \(.title)"'
gh api repos/thomhurst/Spectre.MEL/issues/<N>/dependencies/blocked_by --jq '[.[].number]'
```
...
3. Link the child to the parent as a native sub-issue:
   ...
   ```powershell
   gh api --method POST repos/thomhurst/Spectre.MEL/issues/<N>/sub_issues -F sub_issue_id=$childId
   ```

4. For ordering, mark a child blocked by its prerequisite (use the prerequisite's `.id`):

   ```powershell
   gh api --method POST repos/thomhurst/Spectre.MEL/issues/<blockedNum>/dependencies/blocked_by -F issue_id=<blockingId>
   ```
...
Search for the API, type, logger/formatter/renderer path, or console scenario the issue references. If it does not exist, the issue is asking you to build it; that is the true scope.

Use `rg` across source, tests, benchmarks, and samples before estimating scope:

```powershell
rg "<symbol|type|option>" src tests benchmarks samples -g "*.cs" -g "*.md" -g "*.json" -g "*.yml"
```
